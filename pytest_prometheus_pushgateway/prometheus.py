...
class PrometheusReport:
    ...
    def _make_metric_name(self, name: str):
    ...
    def pytest_sessionfinish(self, session: Session, exitstatus: Union[int, ExitCode]):
        ...
        added_labels = session.config.hook.pytest_metrics_add_labels(session=session, exitstatus=exitstatus)
        if added_labels:
            labels = {**added_labels[0], **default_labels}
        else:
            labels = default_labels
        i = Info(os.environ.get("PROMETHEUS_PUSHGATEWAY_JOB"), "Info test", registry=self.registry)
        i.info(labels)
        ...
        error_metric = Gauge(self._make_metric_name("error"),
        ...
        if os.environ.get("PROMETHEUS_PUSHGATEWAY_BASIC_AUTH"):
            push_to_gateway(self.pushgateway_url, registry=self.registry, job=self.job_name, handler=my_auth_handler)
        else:
            push_to_gateway(self.pushgateway_url, registry=self.registry, job=self.job_name)

    def pytest_terminal_summary(self, terminalreporter: TerminalReporter, exitstatus: int, config: Config):
        terminalreporter.write_sep("-", f"metrics sent on Prometheus PushGateway at {self.pushgateway_url}")
